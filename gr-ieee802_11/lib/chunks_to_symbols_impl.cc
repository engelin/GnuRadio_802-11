...
chunks_to_symbols_impl::work(int noutput_items,
	...
	//std::cout << "chunks ninput items : " << ninput_items[0] << "nstream : " << nstreams << std::endl;

/*
	gr_complex *raw_symbol = (std::complex<float>*)(pmt::blob_data(tags[0].value));
	int raw_symbol_len = pmt::blob_length(tags[0].value);

	std::cout << "raw symbol.. " << std::endl;
	for(int n = 0; n < raw_symbol_len; n++) {
		std::cout << raw_symbol << " ";
...
//std::cout << "chunks to symbol" << std::endl;
	for(int i = 0; i < ninput_items[0]; i++) {
		out[i] = mapping[in[i]];
//std::cout << i << " : " << (unsigned int)in[i] << "_ " << out[i] << "  ";
	}
//std::cout << std::endl;

//get_tags_in_range(tags, 0, nitems_read(0), nitems_read(0) + ninput_items[0], pmt::mp("ofdm_start"));
//std::cout << "ofdm start " << std::endl;

	//std::cout << "chunks out items : " << out << std::endl;
