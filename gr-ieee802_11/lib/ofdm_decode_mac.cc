...
int general_work (int noutput_items, gr_vector_int& ninput_items,
	...
	while(i < ninput_items[0]) {
		...
		if(tags.size()) {
//std::cout << cnt << " frame has " << nsym << "-sym" << std::endl;
//std::cout << "decode mac: " << cnt << std::endl;
cnt++;
nsym = 0;
			...
			//ofdm_param ofdm = ofdm_param(QAM16_1_2);
...
//cnt++;
		...
		if(copied < d_tx.n_sym) {
			...
			if(copied == d_tx.n_sym) {
//				dout << cnt << "-th received complete frame - decoding" << std::endl;

/*for (int nn = 0; nn < copied * 48; nn++) {
  dout << sym[nn] << " ";
  if(nn % 48 == 47) dout << nn/48 << std::endl;
} dout << std::endl;*/
				decode();
...
/////////////////////checksum
	...
	//std::cout << "crc.." << cnt << "\n\n" << std::endl;
/////////////////////////////////////////
/*	for(int ss = 0; ss < copied * 48;ss++) {
		std::cout << (gr_complex)sym[ss] << " ";
		if(ss % 48 == 47) std::cout << " /// "<< ss/48 << std::endl;
	} std::cout << std::endl;*/

//std::cout << "encoding.. " << d_ofdm.encoding << std::endl;
/*	std::cout << "demodulate bit : " << bits << std::endl;
	std::cout << "deinter : " << std::endl;
	for (int ss = 0; ss < 48*35; ss++) {
          std::cout << (int)out_bits[ss] << " ";
          if (ss % 48 == 48) std::cout << " /// " << ss/48 << std::endl;
        } std::cout << std::endl;

        std::cout << "length viterbi decoded " << decoded_bits.size() << std::endl;
        std::cout << decoded_bits << std::endl;

	std::cout << "crc.." << std::endl;
	for (int ss = 0; ss < decoded_bits.size(); ss++) {
	  std::cout << (int)out_bits[ss] << " ";
	  if (ss % 24 == 23) std::cout << " /// " << ss/24 << std::endl;
	} std::cout << std::endl;*/
	...
	//pmt::pmt_t blob = pmt::make_blob(out_bytes + 2, d_tx.psdu_size);
	//pmt::pmt_t blob = pmt::make_blob(out_bytes + 2, copied * 6);
	pmt::pmt_t enc = pmt::from_uint64(d_ofdm.encoding);
	pmt::pmt_t dict = pmt::make_dict();
	dict = pmt::dict_add(dict, pmt::mp("encoding"), enc);

        pmt::pmt_t raw_sym = pmt::make_blob(&sym[0], (copied * 8 * 48));
	dict = pmt::dict_add(dict, pmt::mp("raw_symbol"), raw_sym);
...
void demodulate() {
	...
	bits = bits * (-2) + 1;
	dout << "demodulate bit : " << bits << std::endl;
}
...
void decode_conv() {
	...
	std::cout << "length rx_signal " << rx_signal.size() << std::endl;
	std::cout << "length viterbi decoded " << decoded_bits.size() << std::endl;
